...
const META_PACKAGES = new Set(['demo', 'generator-bridge']);
...
function getPackages(packagePattern, tsOnly = false) {
  ...
  if (pattern === '*' && !tsOnly) {
    return `@bridge-ui/!(${[...META_PACKAGES].join('|')})`;
  }
  if (!pattern.includes('*')) {
    pattern = `*${pattern}`;
  }
  const packages = [
    ...new Set(
      fastGlob
        .sync([
          `./node_modules/@bridge-ui/${pattern}/src/**/*.${
...
if (shouldCleanup) {
  ...
  const dirtyModules = 'node_modules/@types/react,node_modules/@bridge-ui';
