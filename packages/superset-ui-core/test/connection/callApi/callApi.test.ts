...
import callApi from '@bridge-ui/core/src/connection/callApi/callApi';
import * as constants from '@bridge-ui/core/src/connection/constants';
import { CallApi, JsonObject } from '@bridge-ui/core/src/connection/types';
import { DEFAULT_FETCH_RETRY_OPTIONS } from '@bridge-ui/core/src/connection/constants';
...
describe('callApi()', () => {
  ...
  describe('caching', () => {
    ...
    it('caches requests with ETags', async () => {
      ...
      expect(calls).toHaveLength(1);
      const bridgeCache = await caches.open(constants.CACHE_KEY);
      const cachedResponse = await bridgeCache.match(mockCacheUrl);
