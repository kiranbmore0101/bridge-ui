...
describe('BridgeClientClass', () => {
  ...
  describe('new BridgeClientClass()', () => {
    it('fallback protocol to https when setting only host', () => {
      const client = new BridgeClientClass({ host: 'TEST-HOST' });
      expect(client.baseUrl).toEqual('https://test-host');
    });
  });

  describe('.getUrl()', () => {
    let client = new BridgeClientClass();

    beforeEach(() => {
      client = new BridgeClientClass({
        protocol: 'https:',
        host: 'CONFIG_HOST',
      });
    });
    ...
    it('does not throw if url, endpoint, and host are all empty', () => {
      client = new BridgeClientClass({ protocol: 'https:', host: '' });
  ...
  describe('.init()', () => {
    ...
    it('calls api/v1/security/csrf_token/ when init() is called if no CSRF token is passed', async () => {
      expect.assertions(1);
      await new BridgeClientClass().init();
    ...
    it('does NOT call api/v1/security/csrf_token/ when init() is called if a CSRF token is passed', async () => {
      expect.assertions(1);
      await new BridgeClientClass({ csrfToken: 'abc' }).init();
    ...
    it('calls api/v1/security/csrf_token/ when init(force=true) is called even if a CSRF token is passed', async () => {
      ...
      const client = new BridgeClientClass({ csrfToken: initialToken });
  ...
  describe('.isAuthenticated()', () => {
    ...
    it('returns true if there is a token and false if not', async () => {
      expect.assertions(2);
      const client = new BridgeClientClass({});
    ...
    it('returns true if a token is passed at configuration', () => {
      expect.assertions(2);
      const clientWithoutToken = new BridgeClientClass({
        csrfToken: undefined,
      });
      const clientWithToken = new BridgeClientClass({ csrfToken: 'token' });
  ...
  describe('requests', () => {
    ...
    it('checks for authentication before every get and post request', async () => {
      ...
      const authSpy = jest.spyOn(BridgeClientClass.prototype, 'ensureAuth');
      const client = new BridgeClientClass({ protocol, host });
    ...
    it('sets protocol, host, headers, mode, and credentials from config', async () => {
      ...
      const client = new BridgeClientClass(clientConfig);
      await client.init();
      await client.get({ url: mockGetUrl });
