...
import {
  ...
  ChartMetadata,
} from '@bridge-ui/core/src/chart';
import { SliceIdAndOrFormData } from '@bridge-ui/core/src/chart/clients/ChartClient';
...
describe('ChartClient', () => {
  ...
  describe('new ChartClient(config)', () => {
    ...
    it('creates a client with specified config.client', () => {
      const customClient = new BridgeClientClass();
  ...
  describe('.loadDatasource(datasourceKey, options)', () => {
    it('fetches datasource', () => {
      fetchMock.get(
        'glob:*/bridge/fetch_datasource_metadata?datasourceKey=1__table',
        {
          field1: 'abc',
          field2: 'def',
        },
