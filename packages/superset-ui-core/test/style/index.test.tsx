...
describe('@bridge-ui/style package', () => {
  it('exports a theme', () => {
    expect(typeof bridgeTheme).toBe('object');
  });
  ...
  it('exports BridgeThemeProps', () => {
    const props: BridgeThemeProps = {
      theme: bridgeTheme,
    };
  ...
  describe('useTheme()', () => {
    it('returns the theme', () => {
      function ThemeUser() {
        expect(useTheme()).toStrictEqual(bridgeTheme);
        return <div>test</div>;
      }
      mount(<ThemeUser />, {
        wrappingComponent: ({ children }) => (
          <EmotionCacheProvider value={emotionCache}>
            <ThemeProvider theme={bridgeTheme}>{children}</ThemeProvider>
          </EmotionCacheProvider>
        ),
      });
    });
