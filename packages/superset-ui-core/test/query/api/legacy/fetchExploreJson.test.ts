...
import { fetchExploreJson } from '@bridge-ui/core/src/query/api/legacy';
...
describe('fetchExploreJson()', () => {
  ...
  it('returns a promise of LegacyChartDataResponse', () => {
    fetchMock.post('glob:*/bridge/explore_json/', {
      field1: 'abc',
      field2: 'def',
    });
  ...
  it('uses GET when specified', async () => {
    expect.assertions(4);
    const mockUrl = 'glob:*/bridge/explore_json/*';
    ...
    expect(mockCalls[0][0]).toEqual(
      'http://localhost/bridge/explore_json/?form_data=%7B%22granularity%22%3A%22minute%22%2C%22viz_type%22%3A%22word_cloud%22%2C%22datasource%22%3A%221__table%22%7D',
    );
