...
import { JsonObject } from '@bridge-ui/core/src/connection';
import handleError, {
  ErrorInput,
} from '@bridge-ui/core/src/query/api/v1/handleError';
import {
  BridgeApiError,
  BridgeApiErrorType,
} from '@bridge-ui/core/src/query/api/v1/types';

async function testHandleError(
  inputError: ErrorInput,
  expected: string | JsonObject,
): Promise<BridgeApiError> {
  try {
    await handleError(inputError);
  } catch (error) {
    const typedError = error as BridgeApiError;
    expect(typedError).toBeInstanceOf(BridgeApiError);
  ...
  return new BridgeApiError({ message: 'Where is the error?' });
}

describe('handleError()', () => {
  it('should throw error directly', async () => {
    expect.assertions(3);
    const input = new BridgeApiError({ message: 'timeout' });
  ...
  it('should handle response of single error', async () => {
    ...
    const mockResponse = new Response(
      '{ "error": "BAD BAD", "link": "https://bridge.apache.org" }',
      {
        status: 403,
    ...
    await testHandleError(mockResponse, {
      message: 'BAD BAD',
      extra: { link: 'https://bridge.apache.org' },
    });
  });
  ...
  it('should handle single error object', async () => {
    ...
    const mockError = {
      error: {
        ...
        error_type: BridgeApiErrorType.FRONTEND_TIMEOUT_ERROR,
      },
    };
