...
import { makeApi, BridgeApiError } from '@bridge-ui/core/src/query';
...
describe('makeApi()', () => {
  ...
  it('should allow custom client', async () => {
    ...
    const client = new BridgeClientClass({ baseUrl: 'http://foo/' });
  ...
  it('should handle errors', async () => {
    ...
    try {
      ...
      expect((error as BridgeApiError).message).toContain('Invalid payload');
    }
  });
  ...
  it('should handle error on 200 response', async () => {
    ...
    try {
      ...
      expect((error as BridgeApiError).message).toContain('not ok');
    }
  });
