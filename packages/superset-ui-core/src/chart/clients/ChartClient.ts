...
export default class ChartClient {
  readonly client: BridgeClientInterface | BridgeClientClass;
  ...
  constructor(
    ...
    const { client = BridgeClient } = config;
    this.client = client;
  }
  ...
  loadDatasource(
    ...
    return this.client
      .get({
        endpoint: `/bridge/fetch_datasource_metadata?datasourceKey=${datasourceKey}`,
        ...options,
      } as RequestConfig)
