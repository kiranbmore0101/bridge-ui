...
import {
  BridgeApiError,
  BridgeApiErrorPayload,
  BridgeApiMultiErrorsPayload,
} from './types';

export type ErrorInput = string | Error | Response | BridgeApiErrorPayload;

/**
 * Handle API request errors, convert to consistent Bridge API error.
 * @param error the catched error from BridgeClient.request(...)
 */
export default async function handleError(error: ErrorInput): Promise<never> {
  ...
  if (typeof error === 'string') {
    throw new BridgeApiError({ message: error });
  }
