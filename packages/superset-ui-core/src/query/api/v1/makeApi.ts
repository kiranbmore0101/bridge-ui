...
interface BridgeApiFactoryOptions extends Omit<RequestBase, 'url'> {
  /**
...
>({
  ...
  async function request(
    payload: Payload,
    { client = BridgeClient }: BridgeApiRequestOptions = {
      client: BridgeClient,
    },
  ): Promise<Result> {
    try {
      ...
      if (responseType === 'text') {
        ...
        if (result && typeof result === 'object' && 'error' in result) {
          return handleError(result as BridgeApiErrorPayload);
        }
      }
