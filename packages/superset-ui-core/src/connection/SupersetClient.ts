...
import BridgeClientClass from './BridgeClientClass';
import { BridgeClientInterface } from './types';

let singletonClient: BridgeClientClass | undefined;

function getInstance(): BridgeClientClass {
  if (!singletonClient) {
    throw new Error(
      'You must call BridgeClient.configure(...) before calling other methods',
    );
  }
  return singletonClient;
}

const BridgeClient: BridgeClientInterface = {
  configure: config => {
    singletonClient = new BridgeClientClass(config);
    return singletonClient;
  },
  reset: () => {
