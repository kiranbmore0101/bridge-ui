...
export default class BridgeClientClass {
  ...
  isAuthenticated(): boolean {
    // if CSRF protection is disabled in the Bridge app, the token may be an empty string
  ...
  async ensureAuth(): CsrfPromise {
    return (
      ...
      Promise.reject({
        error: `BridgeClient has not been provided a CSRF token, ensure it is
