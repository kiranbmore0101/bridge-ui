...
export default async function callApi({
  ...
  if (
    ...
    const bridgeCache = await caches.open(CACHE_KEY);
    const cachedResponse = await bridgeCache.match(url);
    if (cachedResponse) {
    ...
    if (response.status === HTTP_STATUS_NOT_MODIFIED) {
      const cachedFullResponse = await bridgeCache.match(url);
    ...
    if (response.status === HTTP_STATUS_OK && response.headers.get('Etag')) {
      ...
      bridgeCache.put(url, response.clone());
    }
