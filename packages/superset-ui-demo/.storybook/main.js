...
// find @bridge-ui packages
const basePath = path.resolve(__dirname, '../../../node_modules/@bridge-ui');
  ...
  webpackFinal: config => {
    ...
    Object.assign(config.resolve.alias, {
      ...packages.reduce(
        (acc, name) => ({
          ...acc,
          [`@bridge-ui/${name}$`]: path.join(basePath, name, 'src'),
        }),
        {},
      ),
    });

    // todo: remove hard code after move storybook to bridge repo.
