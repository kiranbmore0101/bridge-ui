...
import { smartDateFormatter, getNumberFormatter } from '@bridge-ui/core';
...
export function cleanColorInput(value) {
  // for bridge series that should have the same color
  return String(value)
    .trim()
    .replace(' (right axis)', '')
    .split(', ')
...
export function generateAreaChartTooltipContent(
  ...
  d.series.forEach(series => {
    ...
    let trClass = '';
    if (series.highlight) {
      trClass = 'bridge-legacy-chart-nvd3-tr-highlight';
    } else if (series.key === 'TOTAL') {
      trClass = 'bridge-legacy-chart-nvd3-tr-total';
    }
    tooltip +=
